...
const dogsChecked = checkDogs(juliaData, kateData);
dogsChecked.forEach((age, i) => {
	const name = `Dog number ${i + 1} `;
	const message =
		age >= 3
...
const calcAverageHumanAge = ages => {
	const calAge = age => (age > 2 ? 16 + age * 4 : age * 2);
	return (
		ages
			// .map(age => (age > 2 ? 16 + age * 4 : age * 2))
			// .filter(age => age >= 18)
			.reduce((result, age) => {
				const finalAge = calAge(age);
				if (finalAge >= 18) {
					result.push(finalAge);
				}
				return result;
			}, [])
...
in ascending order (keep in mind that the portions are inside the array's objects)
HINT 1: Use many tools to solve these challenges, you can use the summary
...
console.log('sarahDog:', sarahsDog);

// 3
/*const ownersEatTooMuch = dogs
	.filter(
		({ curFood, owners, recommendedFood }) =>
			curFood > recommendedFood
	)
	.flatMap(({ owners }) => owners);*/

// performance way
const ownersEatTooMuch = dogs.reduce((result, dog) => {
	if (dog.curFood > dog.recommendedFood) {
		result.push(...dog.owners);
	}
	return result;
}, []);
...
// performance way
const ownersEatTooLittle = dogs.reduce((result, dog) => {
	if (dog.curFood < dog.recommendedFood) {
		result.push(...dog.owners);
	}
	return result;
...
// 4
const ownersEatTooMuchStr = ownersEatTooMuch.join(' and ');
const ownersEatTooLittleStr = ownersEatTooLittle.join(' and ');
console.log(`${ownersEatTooMuchStr}'s dogs eat too much!`);
console.log(`${ownersEatTooLittleStr}'s dogs eat too little!`);
